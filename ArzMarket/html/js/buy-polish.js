(() => {
  ...
  function classifyFields() {
    ...
    const orderedLabels = ['цена sa$', 'количество', 'цена vc$', 'осталось'];
    for (const label of orderedLabels) {
      const field = byLabel.get(label);
      if (field) main.appendChild(field);
    }

    const statusToggle = toggles.find(node => normalizeLabel(node.textContent).includes('статус товара'));
